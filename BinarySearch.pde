...
{
  int yee = (nHigh+nLow)/2;
  if (nLow > nHigh){
    return -1;
  } else if (store[yee].getCatNum() == catNumToFind){
    return store[yee].getInventory();
  } else if (store[yee].getCatNum() < catNumToFind){
    return recursiveBinarySearch(catNumToFind, yee +1, nHigh);
  } else {
